...
class DirectoryInstallTarget(InstallTarget):
    ...
    def setup_subparser(self, parser: typing.Any) -> None:
        ...
        parser.add_argument(
            "--mode", action="store", default=0, type=int, help="mode of exported file."
        )
        parser.add_argument("--owner", action="store", help="owner of exported file")
        parser.add_argument("--group", action="store", help="group of exported file")

        parser.add_argument(
            "--create-target-directory", dest="create_directory", action="store_true"
        )
