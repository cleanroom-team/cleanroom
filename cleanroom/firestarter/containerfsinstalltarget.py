...
def _extract_into_snapshot(_, rootfs: str, *, import_snapshot: str) -> None:
    ...
    tool.run('/usr/bin/bash', '-c',
             '( cd {} ; tar -cf - . ) | ( cd {} ; tar -xf - )'
             .format(rootfs, import_snapshot))


class ContainerFilesystemInstallTarget(InstallTarget):
    def __init__(self) -> None:
        super().__init__('container_fs',
                         'Install a container filesystem.')
    ...
    def __call__(self, parse_result: typing.Any) -> None:
        ...
        try:
            ...
            tool.execute_with_system_mounted(
                lambda e, r:
                    _extract_into_snapshot(e, r, import_snapshot=import_dir),
    ...
    def setup_subparser(self, parser: typing.Any) -> None:
        parser.add_argument('--container-name',
                            dest='override_system_name', action='store',
                            nargs='?', default='',
