...
class ImagePartitionInstallTarget(InstallTarget):
    ...
    def setup_subparser(self, parser: typing.Any) -> None:
        parser.add_argument('--efi-device', action='store',
                            default='/dev/sda1', dest='efi_device',
                            help='The device containing the EFI partition.')

        parser.add_argument('--image-device', action='store',
                            default='', dest='image_device',
                            help='The device containing the images.')
        parser.add_argument('--image-fs-type', action='store',
                            default='btrfs', dest='image_fs_type',
                            help='The filesystem type containing the image '
                                 '[defaults to btrfs].')
        parser.add_argument('--image-options', action='store',
                            default='subvolid=.images', dest='image_options',
        ...
        with TemporaryDirectory() as tempdir:
            with mount.Mount(parse_result.image_device,
                ...
                exported_file = tool.export_into_directory(
                        parse_result.system_name, images_mnt,
