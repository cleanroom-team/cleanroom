...
def debootstrap(system_context: SystemContext, *, suite: str, variant: str='',
    ...
    shutil.move(_dpkg_config_directory(system_context, internal=True),
    ...
    os.makedirs(os.path.join(_apt_config_directory(system_context, internal=False),
                'preferences.d'))

    # Copy over trust db if none exists yet:
    # FIXME: Create a new one?
    gpg_dir = os.path.join(_apt_config_directory(system_context, internal=False),
                           'trusted.gpg.d')
