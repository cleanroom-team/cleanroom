...
class K8sNodeCommand(Command):
    ...
    def _setup_network(self, location, system_context, *,
                       cluster_name, cluster_id, node_id,
                       outside_match, cluster_match,
                       gateway, dns, ntp):
...
'''.format(cluster_name=cluster_name, cluster_match=cluster_match),
    ...
    def _install_packages(self, location, system_context):
    ...
    def _setup_docker(self, location, system_context):
        system_context.execute(location, 'mkdir', '/usr/lib/systemd/system/docker.service.d')
        system_context.exceute(location, 'create', '/usr/lib/systemd/system/docker.service.d/override.conf', '''[Service]
ExecStart=
...
''', mode=0o644)
    ...
    def _setup_kubelet(self, location, system_context, *, master_ip, node_ip):
...
'''.format(master_ip=master_ip, node_ip=node_ip), mode=0o644)
    ...
    def _setup_kube_proxy(self, location, systtem_context, *, master_ip):
        system_context.execute(location, 'mkdir',
...
'''.format(master_ip=master_ip), mode=0o644)
    ...
    def __call__(self, location, system_context, *args, **kwargs):
        ...
        self._setup_network(location, system_context,
        ...
        self._setup_docker(location, system_context)

        self._setup_kubelet(location, system_context, master_ip=master_ip, node_ip=node_ip)
        self._setup_kube_proxy(location, system_context)
