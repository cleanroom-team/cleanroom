...
def _setup_network(location: Location, system_context: SystemContext, *,
    ...
    create_file(system_context, '/usr/lib/systemd/network/20-cbr0-bridge.netdev',
                textwrap.dedent('''\
                [NetDev]
    ...
    create_file(system_context, '/usr/lib/systemd/network/10-extern.network',
                textwrap.dedent('''\
                [Match]
                {outside_match}
                ...
                ''').format(outside_match=outside_match,
                            ...
                            gateway=gateway, dns=dns, ntp=ntp).encode('utf-8'),
                mode=0o644)

    create_file(system_context, '/usr/lib/systemd/network/30-cbr.network',
                textwrap.dedent('''\
                [Match]
                Name=cbr0
    ...
    create_file(system_context, '/usr/lib/systemd/network/40-cbr-outside-if.network',
                textwrap.dedent('''\
                [Match]
                {cluster_match}
...
def _setup_docker(location: Location, system_context: SystemContext) -> None:
    ...
    create_file(system_context, '/usr/lib/systemd/system/docker.service.d/override.conf',
                textwrap.dedent('''\
                [Service]
                ExecStart=
...
def _setup_kubelet(location: Location, system_context: SystemContext, *,
    ...
    create_file(system_context, '/usr/lib/tmpfiles.d/kubelet.conf',
                'd /var/lib/kubelet 0700 - - -'.encode('utf-8'), mode=0o644)

    os.makedirs('/usr/lib/systemd/system/kubelet.service.d')
    create_file(system_context, '/usr/lib/systemd/system/kubelet.service.d/override.conf',
                textwrap.dedent('''\
                [Service]
                EnvironmentFile=
                ExecStart=
                ...
                ''').format(master_ip=master_ip, node_ip=node_ip).encode('utf-8'),
                mode=0o644)
...
def _setup_kube_proxy(location: Location, system_context: SystemContext, *,
    ...
    create_file(system_context, '/usr/lib/systemd/system/kube-proxy.service.d/override.conf',
                textwrap.dedent('''\
                [Service]
                EnvironmentFile=
                ExecStart=
...
class K8sNodeCommand(Command):
    ...
    def _install_packages(self, location: Location,
                          system_context: SystemContext) -> None:
        self._execute(location, system_context,
                      'pacman', 'docker', 'kubernetes-bin')

        # Debug packages: Remove again:
        self._execute(location, system_context,
                      'pacman', 'wget', 'net-tools', 'gnu-netcat')
