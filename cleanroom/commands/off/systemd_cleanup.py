...
def _map_base(old_base: str, new_base: str, input_path: str) \
...
def _map_target_link(old_base: str, new_base: str,
                     link: str, link_target: str) -> typing.Tuple[str, str]:
    assert old_base.endswith('/')
    ...
    (link, old_link) \
        = _map_base(old_base, new_base, link)
    (link_target, old_link_target) \
        = _map_base(old_base, new_base,
...
def _map_host_link(root_directory, old_base, new_base,
                   link, link_target):
    ...
    (host_link, link_target) \
        = _map_target_link(host_old_base, host_new_base,
                           host_link, link_target)
...
def _move_file(location, old_base, new_base, path):
    ...
    if os.path.exists(new_path):
        raise GenerateError('"{}" already exists when moving "{}".'
        ...
        for root, dirs, files in os.walk(old_base):
            for f in files:
                ...
                if os.path.islink(full_path):
                    ...
                    _move_symlink(location, system_context,
