...
def test_group_data(user_setup, group_name, expected_data):
...
def test_missing_group_data_file(user_setup):
...
def test_add_group(user_setup):
    ...
    assert result._asdict() == {'name': 'addedgroup', 'password': 'x',
                                'gid': 1200, 'members': []}


def test_mod_group(user_setup):
    binary_manager = BinaryManager()
    group_helper = GroupHelper(binary_manager.binary(Binaries.GROUPADD),
                               binary_manager.binary(Binaries.GROUPMOD))
    ...
    assert result._asdict() == {'name': 'tester', 'password': 'x', 'gid': 10001,
