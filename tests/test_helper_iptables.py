...
def _validate_rules_files(system_context, *extra_matches) -> None:
    _validate_rules_file(system_context.file_name('/etc/iptables/iptables.rules'),
                         *extra_matches)
    _validate_rules_file(system_context.file_name('/etc/iptables/ip6tables.rules'),
                         *extra_matches)
...
def _validate_rules_file(file_name: str, *extra_matches) -> None:
    ...
    for match in (*extra_matches,
                  ipt._FORWARD_MAGIC, ipt._TCP_MAGIC, ipt._UDP_MAGIC):
        assert ('\n' + match + '\n') in rules_contents
